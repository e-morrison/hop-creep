...
def open_file(sql_file):
    sql_file = f'data/merops/{sql_file}'
    with open(sql_file, 'rb') as file:
        raw_data = file.read()
        detected_encoding = chardet.detect(raw_data)['encoding']

    with open(sql_file, 'r', encoding=detected_encoding) as file:
        return file.read()
...
if __name__ == "__main__":
    #sql_files = [file1 for file1 in os.listdir('data/merops/') if file1.endswith('.sql')]
    sql_files = [
                    'Substrate_search.sql',
                    'residue_codes.sql',
                    ]
